...
## Các khía cạnh chính của quản lý variable

| Khía cạnh                         | Giải thích đơn giản                                                                                                               |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| **Biết nhiệm vụ của nó**         | Mỗi variable nên có một trách nhiệm rõ ràng.                                                                                       |
| **Dùng đúng type**               | Dùng type GTM đơn giản nhất có thể giải quyết vấn đề.                                                                              |
| **Đặt tên rõ ràng**              | Tên phải mô tả dữ liệu chứa bên trong và, khi hữu ích, cả nguồn dữ liệu.                                                           |
| **Tái sử dụng**                  | Tạo một variable dùng chung cho cùng một thông tin thay vì tạo bản sao.                                                           |
| **Ưu tiên Data Layer**           | Đọc dữ liệu application cung cấp qua Data Layer thay vì scrape DOM.                                                               |
| **Giữ thay đổi đơn giản**        | Để business logic và transformation phức tạp trong application; GTM chủ yếu đọc value và làm mapping đơn giản.                  |
| **Tách environment**             | Tách rõ cấu hình tracking cho QA, staging và production.                                                                          |
| **Xử lý dữ liệu thiếu**          | Quy định khi value bắt buộc bị thiếu: bỏ qua, block tag hay dùng default đã được phê duyệt.                                      |
| **Bảo vệ người dùng**            | Không expose PII, credential, token, secret hoặc dữ liệu nhạy cảm khác qua GTM variable.                                         |
| **Test và dọn dẹp**              | Test trước release, document nơi được sử dụng và retire variable không còn cần thiết.                                           |

### Biết nhiệm vụ của variable

Mỗi GTM variable nên có một trách nhiệm rõ ràng và dễ dự đoán.

Variable nên trả lời một câu hỏi đơn giản như:

```text
Loại connection hiện tại là gì?
```

hoặc:

```text
Trạng thái solution hiện tại là gì?
```
...
chỉ nên đọc từ Data Layer:

```text
solution_found
```

Nó không nên đồng thời:

- đổi tên value;
- đọc text từ DOM;
- quyết định dùng Measurement ID nào;
- áp dụng business rule không liên quan;
- hoặc kết hợp nhiều value không liên quan.

Giữ variable tập trung giúp dễ hiểu, test, debug và reuse hơn.

Variable tốt hoạt động như sau:
...
Thiết kế không tốt khiến một variable phải làm nhiều việc không liên quan:

```text
Đọc solution_found
+ kiểm tra hostname
+ đổi tên value
+ chọn GA4 Measurement ID
+ áp dụng fallback logic
```

Điều này làm variable khó bảo trì vì một thay đổi nhỏ có thể ảnh hưởng nhiều behavior tracking.

Quy tắc hữu ích:

> Một variable nên đại diện cho một thông tin rõ ràng hoặc một transformation đơn giản.

### Dùng đúng type

Chọn native GTM variable type đơn giản nhất phù hợp với requirement.

GTM đã có nhiều variable type cho các tác vụ phổ biến. Hãy dùng chúng trước khi tạo Custom JavaScript.

| Requirement                                       | GTM variable type khuyến nghị |
| ------------------------------------------------- | ------------------------------ |
| Đọc `inputs.connection_type` từ Data Layer        | Data Layer Variable            |
| Lưu Measurement ID cố định                        | Constant                       |
| Mapping hostname sang Measurement ID              | Lookup Table                   |
| Đọc pathname, hostname hoặc query parameter      | URL Variable                   |
| Đọc value từ DOM của page                         | DOM Element Variable           |
| Logic native GTM không xử lý được                 | Custom JavaScript              |

Ví dụ, các domain khác nhau dùng GA4 Measurement ID khác nhau:
...
Có thể xử lý bằng Lookup Table:
...
Cả hai cách đều có thể hoạt động, nhưng Lookup Table dễ đọc và bảo trì hơn.

Nguyên tắc:

> Không dùng variable type phức tạp hơn khi GTM đã có native type đơn giản cho cùng nhiệm vụ.

Custom JavaScript thường nên là lựa chọn cuối cùng, không phải mặc định.

### Đặt tên rõ ràng

Tên GTM variable nên cho biết:

- variable thuộc project hoặc application nào;
- variable type là gì;
- value bên trong là gì;
- và, khi hữu ích, value đến từ đâu.
...
Tên đầu tiên cho developer biết ngay:

```text
FD
→ thuộc application FD

DLV
→ Data Layer Variable

inputs
→ đến từ object inputs

connection_type
→ chứa loại connection
```

Quy ước nhất quán cũng giúp tìm kiếm và nhóm variable dễ hơn.
...
Điều này đặc biệt hữu ích trong container có nhiều tag, trigger và variable.
...
Các tên này cung cấp ít thông tin và sẽ khó hiểu về sau.

Mục tiêu chính:

> Người không tạo variable vẫn phải hiểu được purpose của nó chỉ từ tên.

### Tái sử dụng variable

Tạo một GTM variable canonical cho một business concept dùng chung.

Nếu nhiều tag cần cùng một value, thông thường chúng nên reuse cùng một variable.
...
nếu cả ba variable đều đọc chính xác cùng một giá trị Data Layer.

Variable trùng lặp tạo ra nhiều vấn đề:

- nhiều cấu hình hơn cần bảo trì;
- rủi ro setting không nhất quán cao hơn;
- debug khó hơn;
- migration khó hơn;
- không rõ variable nào là canonical.

Một variable mới hợp lý khi có khác biệt thực sự, chẳng hạn:

- source khác nhau;
- expected type khác nhau;
- fallback khác nhau;
- business meaning khác nhau;
- hoặc cần transformation.

Nguyên tắc:

> Một business value dùng chung thông thường chỉ nên có một GTM variable dùng chung.

### Ưu tiên Data Layer

Khi application đã biết một value, hãy cung cấp value đó qua Data Layer thay vì yêu cầu GTM tự tìm trên page.
...
Application đã biết giá trị nội bộ:

```text
wood_to_wood
```

Application có thể push:
...
Tracking dựa trên DOM có thể hỏng khi UI thay đổi.
...
Business concept không đổi, nhưng DOM-based tracking có thể ngừng hoạt động.

Data Layer cung cấp contract ổn định giữa application và GTM.

Nguyên tắc:

> Nếu application đã biết value, hãy expose value đó qua Data Layer thay vì scrape từ UI.

### Giữ thay đổi đơn giản

Business rule và transformation phức tạp thông thường nên được xử lý trong application, không nên dựng lại trong GTM.
...
nhưng analytics cần giá trị ổn định:

```text
clt_floor_floor_half_lap_joint
```

Application nên cung cấp trực tiếp giá trị ổn định:
...
Tránh duplicate business logic trong nhiều GTM variable. Khi business rule thay đổi, phải cập nhật nhiều nơi và dễ tạo analytics không nhất quán.
...
GTM vẫn có thể làm các tác vụ cấu hình đơn giản như:

- hostname mapping;
- Lookup Table đơn giản;
- environment routing;
- value pass-through;
- formatting đơn giản.
...
> Giữ business logic gần application sở hữu dữ liệu và để GTM tập trung vào tracking configuration.

### Tách environment

Variable có value phụ thuộc environment phải được cấu hình rõ để development, QA, staging và production dùng đúng value.

Ví dụ, nếu cùng một GTM container được dùng cho staging và production, không nên hard-code GA4 Measurement ID trong từng tag.

Thay vào đó, tạo variable dùng chung trả về Measurement ID đúng dựa trên hostname hiện tại.

Lookup Table có thể dùng:
...
Các tag liên quan có thể reuse cùng variable:

```text
{{FD - LUT - GA4 Measurement ID}}
```

Cách này gom cấu hình theo environment vào một nơi và giảm nguy cơ gửi traffic staging/test tới GA4 production.

Tránh dùng production value làm fallback tự động:
...
Hostname không xác định có thể vô tình gửi dữ liệu vào production.
...
Quy tắc này đặc biệt hữu ích khi **một GTM container được dùng chung cho nhiều environment**.

Nếu staging và production đã dùng các GTM container tách biệt, mỗi container có Measurement ID riêng, hostname Lookup Table có thể không cần thiết. Requirement quan trọng vẫn là value phụ thuộc environment phải được tách và kiểm soát rõ ràng. Hostname Lookup Table là routing logic, không thay thế GTM Environments — tính năng dùng để publish các container version cụ thể tới development, QA hoặc live environment.

### Xử lý dữ liệu thiếu

Mỗi variable quan trọng phải có behavior rõ ràng khi value không có sẵn.

Giả sử Data Layer contract yêu cầu:

```text
inputs.connection_type
```

nhưng application push:
...
Lúc này GTM không nên tự ý quyết định. Behavior kỳ vọng phải được định nghĩa từ trước.

Ba chiến lược phổ biến:

| Chiến lược       | Khi dùng                                           | Ví dụ                                                            |
| ---------------- | -------------------------------------------------- | ---------------------------------------------------------------- |
| Omit             | Parameter là tùy chọn                              | Không gửi `product_category` khi không có value                 |
| Block            | Value bắt buộc để event hợp lệ                     | Không fire calculation tag khi thiếu `connection_type`           |
| Default phê duyệt | Business đã định nghĩa default thực sự             | Dùng `language = en` chỉ khi `en` là fallback được phê duyệt     |

Tránh âm thầm đổi value bắt buộc bị thiếu thành:

```text
unknown
```

Ví dụ `connection_type = undefined` thành `connection_type = unknown` có thể che giấu vấn đề Data Layer.

GA4 sau đó có thể hiển thị:
...
trông như dữ liệu tracking hợp lệ dù application không cung cấp field bắt buộc.

Với field bắt buộc, tốt hơn là để QA phát hiện lỗi:

```text
Variable bắt buộc bị thiếu
        ↓
Tag bị block
        ↓
QA failure
...
Field tùy chọn có thể được bỏ qua khi event contract cho phép.

Nguyên tắc:

> Dữ liệu thiếu phải có behavior rõ ràng; không bao giờ xử lý tình cờ hoặc âm thầm.

### Bảo vệ người dùng

GTM variable không được expose thông tin cá nhân, nhạy cảm, mật hoặc liên quan bảo mật.

Tránh tạo variable chứa:
...
Ví dụ không nên push vào Data Layer:
...
Và GTM không nên tạo variable như:
...
Ngay cả khi value chưa được chủ ý gửi tới GA4, expose nó vào tracking layer vẫn tạo rủi ro privacy và security không cần thiết.

Thay vào đó, analytics nên dùng business information không nhạy cảm khi phù hợp:
...
> Chỉ expose dữ liệu cần thiết, đã được phê duyệt và an toàn cho analytics.

### Test và dọn dẹp

Quản lý variable không kết thúc khi variable được tạo.

Mỗi variable phải được test trước release và review trong suốt lifecycle.

Trong GTM Preview, test các scenario như:
...
```text
Variable trả về value nào?
Tag nào consume variable?
Tag có fire đúng không?
Điều gì xảy ra khi value bị thiếu?
Behavior có khớp Data Layer contract không?
```

Variable dùng chung quan trọng cũng nên có documentation cơ bản.

Metadata hữu ích có thể gồm:
...
Variable cũng cần được dọn dẹp khi không còn cần.

Luồng retirement an toàn:

```text
Xác định variable không dùng
    ↓
Kiểm tra mọi Tag và Trigger
    ↓
Xác nhận replacement nếu có
    ↓
Đánh dấu deprecated
    ↓
Test
    ↓
Xóa trong GTM release đã review
```

Tránh để lại nhiều variable như:
...
Variable không dùng làm container phức tạp hơn và khiến thay đổi sau này khó hơn.

Nguyên tắc:

> Variable cần có lifecycle: tạo, test, document, reuse, review, deprecate và remove.
